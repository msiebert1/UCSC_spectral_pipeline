...
def main():
    ...
    iraf.onedspec(_doprint=0)
    iraf.specred(_doprint=0)
    iraf.disp(inlist='1', reference='1') #why?
    ...
    if not os.path.isdir('pre_reduced/'):
        ...
        for img in listfile:
            ...
            # overscan
            if not img.startswith('o'):
                if os.isfile('pre_reduced/o'+img):
    ...
    if mkarc != 'n':
        if len(list_arc_b) == 1:
            ...
            for arc in list_arc_b:
                arc_str = arc_str + 'pre_reduced/t'+ arc + ','
            if os.isfile('pre_reduced/ARC_blue.fits'):
        ...
        if len(list_arc_r) == 1:
            ...
            for arc in list_arc_r:
                arc_str = arc_str + 'pre_reduced/t'+ arc + ','
            if os.isfile('pre_reduced/ARC_red.fits'):
    ...
    if mkflat != 'n':
        ...
        if len(list_flat_b) == 1:
            ...
            for flat in list_flat_b:
                flat_str = flat_str + 'pre_reduced/t'+ flat + ','
            #subsets = 'no'
            if os.isfile('pre_reduced/tFlat_blue'):
                os.remove('pre_reduced/tFlat_blue')
            iraf.flatcombine(flat_str, output='pre_reduced/tFlat_blue', 
                             ccdtype='',rdnoise=3.7, 
                             subsets='yes', process='no')
            Flat_blue = 'Flat_blue.fits'
            
        #What is the output here? Check for overwrite
        iraf.specred.response('pre_reduced/t'+Flat_blue, 
                               normaliz='pre_reduced/t'+Flat_blue, 
        ...
        if len(list_flat_r) == 1:
            ...
            for flat in list_flat_r:
                flat_str = flat_str + 'pre_reduced/t'+ flat + ','
            if os.isfile('pre_reduced/tFlat_red'):
                os.remove('pre_reduced/tFlat_red')
            iraf.flatcombine(flat_str, output='pre_reduced/tFlat_red', 
                             ccdtype='', rdnoise=3.8, subsets='yes', process='no')
            Flat_red = 'Flat_red.fits'

        #What is the output here? Check for overwrite
        iraf.specred.response('pre_reduced/t'+Flat_red, 
                              normaliz='pre_reduced/t'+Flat_red, 
...
if __name__ == '__main__':
  main()
>>>>>>> d147c3f851cddb7adada43e9f26fed50211ce537
